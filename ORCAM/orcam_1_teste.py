...
#Função onde está definido todas as informações de envio de e-mail
#
#	emailfrom = "ORCAM_report@maxionwheels.com"
#	try:
#		msgfinal = ''.join(body)
...
def app(cnn, cursor):
	...
	for row in cursor.fetchall():
		...
		for owner in cursor.execute("select * from orcam INNER JOIN Grupos ON orcam.Grupo = Grupos.Grupo INNER JOIN ccustob ON ccustob.CcustoB = orcam.Ccusto where orcam.Grupo=? and anomes=?", row.Grupo, cdate):
			try:
				percFor = (owner.Vr_Budget / 100) * 80
			except TypeError:
				percFor = 0
			if owner.Vr_real > percFor and owner.Vr_Budget > 0:
				perc = (owner.Vr_real / owner.Vr_Budget) * 100
				print "- Conta: " + owner.Grupo + " CC: " + owner.Ccusto + u" Ano/Mês: " + owner.Anomes
				sub = u"ORÇAM: Projetado x Real - Aço"
				body.append(u"""<html> <head> <font face="Calibri" size="3" color = "black"> </head><body> <b>- CC: </b>""" + owner.Ccusto[-3:] + u""" - """ + str(owner[19].encode('ascii', 'ignore')) + u"""		<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; - Previsto:</b> """ + str("%0.2f" % (owner.Vr_Budget)).replace('.',',') + """&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>- Real:</b> """ + str("%0.2f" % (owner.Vr_real)).replace('.',',') +  u"""<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;%</b>: """ + str(perc).split('.')[0] + u""" % </font></body></html>""")
	...
	for row in cursor.fetchall():
		...
		for owner in cursor.execute("select * from orcam INNER JOIN Ccusto ON orcam.Ccusto = Ccusto.Ccusto INNER JOIN grupos ON grupos.Grupo = orcam.Grupo where orcam.Ccusto=? and anomes=?", row.Ccusto, cdate):
			...
			if owner.Vr_real > percFor and owner.Vr_Budget > 0:
				...
				sub = u"ORÇAM: Projetado x Real - Aço"
				body.append("""<html> <head> <font face="Calibri" size="3" color = "black"> </head><body> <b>- Conta: </b>""" + owner.Grupo[-3:] + """ - """ + str(owner[17].encode('ascii', 'ignore')) +  u""" <b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; - Previsto:</b> """ + str("%0.2f" % (owner.Vr_Budget)).replace('.',',') + """&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>- Real:</b> """ + str("%0.2f" % (owner.Vr_real)).replace('.',',') +  u"""<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;%</b>: """ + str(perc).split('.')[0] + u""" % </font></body></html>""")
		if body:
			sent = cursor.execute("""DECLARE @AllValues VARCHAR(4000)
									SELECT @AllValues = COALESCE(@AllValues + ';', '') + email 
